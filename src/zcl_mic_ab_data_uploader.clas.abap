CLASS zcl_mic_ab_data_uploader DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_mic_ab_data_uploader IMPLEMENTATION.
...
*        output =
  ...
  METHOD fill_master_data.
    data : lt_bp type table of zmic_ab_bpa,
           lt_prod type table of zmic_ab_product.
   ...
   append value #(
    ...
    append value #(
     ...
     insert zmic_ab_bpa from table @lt_bp.
     insert zmic_ab_product from table @lt_prod.
  ENDMETHOD.
  ...
  METHOD fill_transaction_data.
    data : o_rand type REF TO cl_abap_random_int,
           ...
           lv_ord_id type zmic_ab_dte_id,
           lt_so type table of zmic_ab_so_hdr,
           lt_so_i type table of zmic_ab_so_item.
    ...
    select * from zmic_ab_bpa into table @data(lt_bpa).
    select * from zmic_ab_product into table @data(lt_prod).
    ...
    insert zmic_ab_so_hdr from table @lt_so.
    insert zmic_ab_so_item from table @lt_so_i.

  ENDMETHOD.

  METHOD flush.
    delete from : zmic_ab_bpa, zmic_ab_product, zmic_ab_so_hdr, zmic_ab_so_item.
  ENDMETHOD.
